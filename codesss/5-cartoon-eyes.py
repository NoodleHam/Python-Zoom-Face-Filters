...
import numpy as np
from helpers import visualize_facial_landmarks, get_biggest_face, get_facial_landmark_dict, get_image_dict_from_hulls, paste_alpha_img, eye_to_cartoon_eye
...
while (True):
    ...
    if len(faces) > 0:
        ...
        for name in ['mouth', 'left_eye', 'right_eye']:
            if name == 'left_eye':
                cartoon_eye = eye_to_cartoon_eye(face_regions[name], eye_bg_img, left_eyeball, final_img=final_img,
                                                 left_eye_loc=landmarks_filter_locs_dict['left_eye'], right_eye_loc=landmarks_filter_locs_dict['right_eye'])
            elif name == 'right_eye':
                pass  # just draw both eyes in left eye
                # cartoon_eye = eye_to_cartoon_eye(face_regions['left_eye'], eye_bg_img, right_eyeball, left_eye=False,
                #                                  final_img=final_img,
                #                                  eye_loc=landmarks_filter_locs_dict[name])
                # use the gaze of just the left eye because otherwise it looks too noisy
            else:
                final_img = paste_alpha_img(final_img, face_regions[name], landmarks_filter_locs_dict[name])
